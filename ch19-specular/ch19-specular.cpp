...
void load_shader()
{
	...
	eyeLoc = glGetUniformLocation(program, "gEyeWorldPos");
	powerLoc = glGetUniformLocation(program, "gMatSpecularPower");
	specularLoc = glGetUniformLocation(program, "gSpecularIntensity");
	gEyeWorldPos = glm::vec3(0.0f, 0.0f, 3.0f);
	gMatSpecularIntensity = 1.0f;
	gSpecularPower = 32;
	
	glUniform1f(powerLoc, gSpecularPower);
	glUniform1f(specularLoc, gMatSpecularIntensity);
}
...
void render()
{
	...
	glUniform3fv(eyeLoc, 1, &gEyeWorldPos[0]);
