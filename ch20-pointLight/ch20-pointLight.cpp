...
Shader pLightShader("diffuse shader");
...
{
	pLightShader.init();
	pLightShader.attach(GL_VERTEX_SHADER, "pointLight.vert");
	pLightShader.attach(GL_FRAGMENT_SHADER, "pointLight.frag");
	pLightShader.link();
	pLightShader.use();
	program = pLightShader.program;
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Tutorial 20 - PointLight");
	GLenum res = glewInit();
	if (res != GLEW_OK)
