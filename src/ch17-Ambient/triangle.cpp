...
void Triangle::Render(const byhj::MvpMatrix &matrix)
{
	...
	glUniform3fv(m_Light.ambient_loc, 1, &m_Light.ambient[0]);
	glUniform3fv(m_Mat.ambient_loc, 1, &m_Mat.ambient[0]);

	glUniformMatrix4fv(mvp_loc, 1, GL_FALSE, &(proj * view * model)[0][0] );
...
void Triangle::init_shader()
{
	...
	m_Light.ambient_loc = glGetUniformLocation(program, "g_Light.ambient");
	...
	assert(m_Light.ambient_loc != INVALID_UNIFORM_LOCATION);
